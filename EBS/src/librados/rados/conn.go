...
// #cgo LDFLAGS: -L${SRCDIR}/../libs/ -lrados
// #include <stdlib.h>
// #include <errno.h>
...
var ConnErrorNotFound = errors.New("Not found")
var ConnErrorAlreadyExist = errors.New("Already exist")
...
func (c *Conn) LookupPool(name string) error {
	c_name := C.CString(name)
	defer C.free(unsafe.Pointer(c_name))
	ret := int(C.rados_pool_lookup(c.cluster, c_name))
	if ret < 0 {
		return RadosError(ret)
	} else {
		return nil
